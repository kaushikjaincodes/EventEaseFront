...
interface MenuItemProps {
  ...
  desc: string;
  enddate: string;
  collaborators?: string; // New prop for collaborators
  onClick?: () => void;
}

const MenuItem: React.FC<MenuItemProps> = ({ desc, label, onClick, enddate, collaborators }) => {
    ...
    <div onClick={onClick} className="cursor-pointer">
      <li className="p-4 hover:bg-gray-700 cursor-pointer">
        ...
        >
          ...
          {collaborators && (
            <p className="text-sm text-neutral-400 mt-2">Collaborators: {collaborators}</p> // Display collaborators
          )}
        </div>
      </li>
    </div>
